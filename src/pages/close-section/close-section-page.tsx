...
import {
  selectNumItemsResolved,
  selectTotalProgress
} from "selectors/num-items-resolved";
    ...
    if (closeSectionPayload) {
      setModulesData(
        keys(closeSectionPayload)
          .filter(
            (key: keyof ModuleEntities) => !isEmpty(closeSectionPayload[key])
          )
          .map(
...
export const ModulesSummary: React.FC<ModulesSummaryProps> = ({
  modulesData
}) => {
  ...
  return (
    <Box column>
      ...
      <Paper>
        <Box
          className={clsx(styles.heading, styles.lightSecondary)}
          align="center">
          <Typography
            color="inherit"
